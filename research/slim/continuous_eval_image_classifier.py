  ...
  with tf.Graph().as_default():
    ...
    names_to_values, names_to_updates = slim.metrics.aggregate_metric_map({
        'Accuracy': slim.metrics.streaming_accuracy(
            predictions, labels_squeezed),
        # 'Recall_5': slim.metrics.streaming_recall_at_k(
        #     logits, labels_squeezed, 5),
    })
    ...
    loss = slim.losses.softmax_cross_entropy(
        logits, labels_one_hot, weights=1.0)
    loss_op = tf.summary.scalar(loss_name, loss, collections=[])
    loss_op = tf.Print(loss_op, [loss], loss_name)
    tf.add_to_collection(tf.GraphKeys.SUMMARIES, loss_op)
    ...
    if not tf.gfile.IsDirectory(FLAGS.checkpoint_path):
      raise ValueError('checkpoint_path should be a dir, not {}'.format(
