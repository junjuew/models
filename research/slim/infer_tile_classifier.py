...
tf.app.flags.DEFINE_string('video_ids',
                           '2.2.2,2.2.4,1.1.7', 'Test video ids')
...
tf.app.flags.DEFINE_string('result_hook', 'PickleDictHook', 'The Hook class used to process inference results. '
                           'One of "RedisHook", "PickleHook".')

tf.app.flags.DEFINE_string(
    'result_file', 'inference_results.p', 'The file to store inference result.')
...
def _create_tf_inference_graph(input_width, input_height, input_mean, input_std, num_classes):
    ####################
    # Input Pipeline   #
    ####################
...
def _evaluate_batch(sess, file_paths, input_images_op, output_endpoints_op, result_hook):
    st = time.time()
    tile_ids = list(itertools.chain.from_iterable(
        [_get_tile_ids_from_file_path(
            image_path, FLAGS.grid_w, FLAGS.grid_h) for image_path in file_paths]))
    ...
    tiles = np.asarray([_divide_to_tiles_fixed_size(
        im=image_np) for image_np in images_np])
    (image_num, tile_per_image,
     tile_h, tile_w, channel_num) = tiles.shape
    assert channel_num == 3
    assert tile_per_image == FLAGS.grid_w * FLAGS.grid_h
    # make them into a batch
    tiles = np.reshape(tiles, (image_num * tile_per_image, tile_h,
                               tile_w, channel_num))
    tf.logging.info('tiling took: {} s'.format(time.time() - st))
    st = time.time()

    outputs = sess.run(output_endpoints_op, feed_dict={
        input_images_op: tiles})
...
def main(_):
    if FLAGS.result_hook == 'RedisHook' and FLAGS.redis_db < 0:
        raise ValueError(
            'Invalid Redis Database index: {}'.format(FLAGS.redis_db))
    if (FLAGS.result_hook in ['PickleHook', 'PickleDictHook']) and (
            not os.path.exists(os.path.dirname(FLAGS.result_file))):
        raise ValueError(
            'Directory does not exist for result file: {}'.format(
                FLAGS.result_file))
    if (FLAGS.grid_w < 0) or (FLAGS.grid_h < 0):
        raise ValueError(
            'Invalid grid_w or grid_h')
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(
            ...
            for file_paths in file_name_iter:
                _evaluate_batch(sess, file_paths, input_images_op,
